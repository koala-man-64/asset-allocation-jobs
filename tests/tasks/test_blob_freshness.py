    ...
    with patch.dict(os.environ, {
        ...
        with patch('monitoring.system_health.AzureBlobStore', return_value=mock_store):
            with patch('monitoring.system_health._default_layer_specs', return_value=[mock_spec]):
                with patch(
                    'monitoring.system_health._marker_probe_config',
                    return_value=system_health.MarkerProbeConfig(
                        enabled=True,
                        container="common",
                        prefix="system/health_markers",
                    ),
                ), patch('monitoring.system_health.collect_container_apps', return_value=[]), \
