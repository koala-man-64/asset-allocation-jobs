...
def test_alpha_vantage_client_failure_is_not_silently_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="missing Alpha Vantage key"):
        bronze._get_active_alpha_vantage_client()
    assert bronze._active_alpha_vantage_client_manager is manager
