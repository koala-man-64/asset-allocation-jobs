...
- repo-local bootstrap now defaults `ASSET_ALLOCATION_API_BASE_URL` to `http://asset-allocation-api` and `JOB_STARTUP_API_CONTAINER_APPS` to `asset-allocation-api`
...
2. Deploy the control-plane first. Jobs now read control data over HTTP and require an internal API base URL. Use `http://asset-allocation-api` for the current same-environment restore path.
...
- If `symbol-cleanup-job` fails before claiming work, treat it as a dependency readiness failure. Check `POSTGRES_DSN`, `ASSET_ALLOCATION_API_BASE_URL`, `ASSET_ALLOCATION_API_SCOPE`, `ASSET_ALLOCATION_API_TIMEOUT_SECONDS`, token acquisition, the internal control-plane route, and the `core.symbol_profiles` / `core.symbol_profile_overrides` table shape.
- If symbol cleanup backlog keeps growing, inspect control-plane queue depth, stale `claimed` items, recent `fail_reported` lifecycle logs, and whether ACA retry replays are repeatedly encountering the same work item. Do not raise `replicaRetryLimit` until completion and failure reporting are confirmed idempotent.
- If failures include `complete_report_failed` or `fail_report_failed`, reconcile the affected work item in the control-plane before manually starting another repair pass; the worker intentionally avoids reporting a failure after a possibly applied completion.
- Track the runtime identity split through the shared Azure owner: `symbol-cleanup-job` should eventually use a pull-only identity for ACR and a separate least-privilege runtime identity for control-plane/API access.
