# Code Drift Sentinel Complete Bundle

This file consolidates the full contents of the `code-drift-sentinel` skill so the agent can be reviewed, shared, or reconstructed from one Markdown artifact.

Included source directory: `.codex/skills/code-drift-sentinel`

Excluded generated files: `scripts/__pycache__/*`

## Included Files

- `.codex/skills/code-drift-sentinel/agents/openai.yaml`
- `.codex/skills/code-drift-sentinel/codedrift_scenarios/01-style-drift.yml`
- `.codex/skills/code-drift-sentinel/codedrift_scenarios/02-dependency-drift.yml`
- `.codex/skills/code-drift-sentinel/codedrift_scenarios/03-api-drift.yml`
- `.codex/skills/code-drift-sentinel/codedrift_scenarios/04-architecture-drift.yml`
- `.codex/skills/code-drift-sentinel/codedrift_scenarios/05-test-drift.yml`
- `.codex/skills/code-drift-sentinel/codedrift_scenarios/06-multi-agent-inconsistency.yml`
- `.codex/skills/code-drift-sentinel/codedrift_scenarios/README.md`
- `.codex/skills/code-drift-sentinel/references/agent.md`
- `.codex/skills/code-drift-sentinel/references/codedrift.schema.json`
- `.codex/skills/code-drift-sentinel/references/codedrift-yml-schema.md`
- `.codex/skills/code-drift-sentinel/scripts/codedrift_sentinel.py`
- `.codex/skills/code-drift-sentinel/scripts/run_scenarios.py`
- `.codex/skills/code-drift-sentinel/SKILL.md`
...
````yaml
interface:
  display_name: "Code Drift Sentinel"
  short_description: "Detect and remediate multi-agent code drift, including speculative safeguards."
  default_prompt: "Use $skill-name to detect code drift, attribute causes, and propose or apply safe remediation. Treat speculative safeguards and placeholder fallbacks such as precautionary null branches or UI copy like 'N/A'/'unknown' as undesired drift unless the requirement explicitly calls for them."
````
...
````yaml
id: style-drift
title: Style drift from formatter/linter mismatch
setup:
  - Introduce formatting and import-order violations in `src/domain/example.ts`.
  - Ensure `standards.format` and `standards.lint` are configured.
run:
...
expected_remediation:
  - Run formatter/lint auto-fixes and re-run style gates.
````
...
````yaml
id: api-drift
title: Public interface breaking change
setup:
  - Modify a function signature in a path matching `api.public_globs`.
  - Remove or rename parameters to simulate a breaking change.
run:
  - python3 .codex/skills/code-drift-sentinel/scripts/codedrift_sentinel.py --mode audit
expected_findings:
  - category: api
    ...
    evidence_contains:
      ...
      - removed
expected_remediation:
  - Add compatibility shim or version the API change.
  - Update contract tests and API docs/changelog.
````
...
````yaml
id: test-drift
title: Behavioral change with missing or removed tests
setup:
  - Modify business logic in source files.
  - Remove or skip corresponding tests, or do not add tests for changed paths.
run:
  - python3 .codex/skills/code-drift-sentinel/scripts/codedrift_sentinel.py --mode audit
expected_findings:
  - category: test
    severity: medium
    evidence_contains:
      - Code changed without nearby test updates
  - category: behavioral
    severity: high
    evidence_contains:
      - Regression signal
expected_remediation:
  - Restore/add tests for changed behavior and edge cases.
  - Re-run fast and full test gates.
````

### `.codex/skills/code-drift-sentinel/codedrift_scenarios/06-multi-agent-inconsistency.yml`

````yaml
id: multi-agent-inconsistency
title: Competing helper abstractions introduced in parallel
setup:
  - Add two helper files in different folders implementing the same function name.
  - Mix error-handling styles (exceptions vs result objects) across changed modules.
run:
  - python3 .codex/skills/code-drift-sentinel/scripts/codedrift_sentinel.py --mode audit
expected_findings:
  - category: architecture
    severity: medium
    evidence_contains:
      - Inconsistent micro-architecture patterns
  - category: architecture
    severity: medium
    evidence_contains:
      - Duplicated utilities
expected_remediation:
  - Choose one canonical helper and migrate call sites.
  - Standardize on the baseline-dominant error handling strategy.
````
...
````json
{
  ...
  "$id": "https://example.local/codedrift.schema.json",
  "title": "Code Drift Sentinel Configuration",
  "type": "object",
  "properties": {
    "baseline": {
      "type": "object",
    ...
    "standards": {
      ...
      "properties": {
        ...
        "lint": { "$ref": "#/$defs/stringList" },
        "typecheck": { "$ref": "#/$defs/stringList" },
        "test_fast": { "$ref": "#/$defs/stringList" },
        "test_full": { "$ref": "#/$defs/stringList" },
        "security": { "$ref": "#/$defs/stringList" },
        "benchmark": { "$ref": "#/$defs/stringList" },
        "format_fix": { "$ref": "#/$defs/stringList" },
        "lint_fix": { "$ref": "#/$defs/stringList" }
      },
      "additionalProperties": true
    },
    "architecture": {
        ...
        "layers": {
          ...
          "items": {
            ...
            "properties": {
              ...
              "forbid_imports_from": { "$ref": "#/$defs/stringList" },
              "allow_imports_from": { "$ref": "#/$defs/stringList" }
            },
            "required": ["name", "path_glob"],
            "additionalProperties": true
          }
        },
        "module_boundaries": { "type": "array" },
        "blessed_patterns": { "$ref": "#/$defs/stringList" }
      },
      "additionalProperties": true
    },
    "dependencies": {
      "type": "object",
      "properties": {
        "allowlist": { "$ref": "#/$defs/stringList" },
        "denylist": { "$ref": "#/$defs/stringList" },
    ...
    "api": {
      "type": "object",
      "properties": {
        "public_globs": { "$ref": "#/$defs/stringList" },
        "breaking_change_policy": { "type": "string", "enum": ["warn", "strict"] }
      },
      "additionalProperties": true
    },
    "thresholds": {
      "type": "object",
      "properties": {
        "drift_score_fail": { "type": "number" },
        "category_weights": { "type": "object", "additionalProperties": { "type": "number" } },
        "category_severity_fail": { "type": "object", "additionalProperties": { "type": "string" } }
      },
      "required": ["drift_score_fail"],
      "additionalProperties": true
    },
    "auto_remediate": {
      "type": "object",
      "properties": {
        ...
        "safe_directories": { "$ref": "#/$defs/stringList" },
        "commands": { "$ref": "#/$defs/stringList" }
      },
      "required": ["enabled", "max_files_changed"],
      "additionalProperties": true
    },
    "risk_controls": {
      "type": "object",
      "properties": {
        "protected_globs": { "$ref": "#/$defs/stringList" },
        "require_tests_passing_before_automerge": { "type": "boolean" }
      },
      "additionalProperties": true
    },
    "reporting": {
