...
- The repo-local bootstrap seeds control-plane connectivity without depending on public ingress discovery: `ASSET_ALLOCATION_API_BASE_URL` defaults to `http://asset-allocation-api-vnet`, while `ASSET_ALLOCATION_API_SCOPE` is still resolved from Azure when possible.
