...
- The repo-local bootstrap seeds control-plane connectivity without depending on public ingress discovery: `ASSET_ALLOCATION_API_BASE_URL` defaults to the internal same-environment service URL `http://asset-allocation-api`, while `ASSET_ALLOCATION_API_SCOPE` is still resolved from Azure when possible. API, UI, and jobs should share `asset-allocation-env` for the approved same-environment repair.
